...
def main():
    args = parse_args()
    # args.cfg = "/home/gjl/MiSLAS/config/cifar10/cifar10_imb01_stage1_mixup.yaml"
    ...
    if config.multiprocessing_distributed:
        ...
        main_worker(config.gpu, ngpus_per_node, config, logger, model_dir)  # 判定单GPU进入


def main_worker(gpu, ngpus_per_node, config, logger, model_dir):
    global best_acc1, its_ece
    config.gpu = gpu                                                        # config.gpu = None?
#     start_time = time.strftime("%Y%m%d_%H%M%S", time.localtime())
    ...
    train_loader = dataset.train_instance # DataLoader以后的一批数据放入train_loader
    val_loader = dataset.eval
    if config.distributed:
        train_sampler = dataset.dist_sampler

    # define loss function (criterion) and optimizer
    criterion = nn.CrossEntropyLoss().cuda(config.gpu) # 实例化Loss为交叉熵损失
...
def train(train_loader, model, classifier, criterion, optimizer, epoch, config, logger, block=None):
    ...
    if config.dataset == 'places':
        ...
        model.train()                   # DataParallel train
    classifier.train()
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if torch.cuda.is_available():
            images = images.cuda(config.gpu, non_blocking=True)         # images是图像，放进gpu
            target = target.cuda(config.gpu, non_blocking=True)         # target是标签，放进gpu
...
# run: python train_stage1.py --cfg /home/gjl/MiSLAS/config/cifar10/cifar10_imb01_stage1_mixup.yaml